export const PROPS_GLOBAL = '__DOMAZE_PROPS';

export const RT_PARENT_VAL_KEY = '__parent';
export const RT_SCOPE_VAL_KEY = '__scope';
export const RT_VALUE_FN_KEY = '__value';

export const RT_ATTR_VAL_PREFIX = 'attr_';
export const RT_CLASS_VAL_PREFIX = 'class_';
export const RT_STYLE_VAL_PREFIX = 'style_';
export const RT_EVENT_VAL_PREFIX = 'on_';
export const RT_TEXT_VAL_PREFIX = 'text_';

export const RT_TEXT_MARKER1_PREFIX = '-t';
export const RT_TEXT_MARKER2 = '-/t';
